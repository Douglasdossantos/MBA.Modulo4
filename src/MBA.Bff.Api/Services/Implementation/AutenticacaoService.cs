...
namespace MBA.Bff.Api.Services.Implementation
{
    ...
    {
        private readonly IAutenticacaoExternalService _autenticacaoService = autenticacaoService;
        ...
        {
            var response = await _autenticacaoService.Login(model);
            if (response == null)
            ...
            try
            {
                var dto = System.Text.Json.JsonSerializer.Deserialize<object>(content);
