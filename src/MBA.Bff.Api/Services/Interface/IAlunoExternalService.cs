...
public interface IAlunoExternalService
{
	...
	Task<HttpResponseMessage> MatricularAluno([Body] MatriculaRequest matriculaViewModel,
		[Header("Authorization")] string authorization = null);

	[Post("/api/Aluno/registrar-aula-assistida")]
	Task<HttpResponseMessage> RegistrarAulaAssistida([Body] AulaAssistidaRequest matriculaViewModel,
		[Header("Authorization")] string authorization = null);
	...
	Task<HttpResponseMessage> AlteraStatusMatricula(Guid matriculaId, int status,
		[Header("Authorization")] string authorization = null);
}